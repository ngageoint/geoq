...
class JobAtom(ListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        fg.title('DRMS Atom Feed for job {}'.format(job.id))
        ...
        # all_aois_features = [json.loads(aoi.grid_geoJSON()) for aoi in self.aois.all()]
        #
        ...
        for loc in locations:
            ...
            #ip = i["properties"]
            #title = "feature id {} -- status: {}, priority: {}".format(ip["id"], ip["status"], ip["priority"])
            title = "ANALYST... template name: {}; analyst name: {}; created at: {}".format(template_name, analyst_name, dtg)
            fe.title(title)
            fe.description(desc)
            fe.link(href="http://www.mitre.org/drms/")
        ...
        for aoi in aois:
            ...
            title = "AOI... id: {}, status: {}; analyst: {}; assignee: {} ({})".format(aoi.id, aoi.status, aoi.analyst, aoi.assignee_name, aoi.assignee_type)
        ...
        return HttpResponse(atom_string, content_type="application/atom+xml", status=200)
